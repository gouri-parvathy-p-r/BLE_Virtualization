...
from device_managers import start_virtual_device, update_ble_peripheral  # Launch BLE_Peripheral.py
...
print(f"[AGENT] Starting main agent loop...")

mqtt = MqttClient(MQTT_BROKER, AGENT_HOSTNAME)
mqtt.connect()


from device_managers import update_ble_peripheral  # New helper

import json
from device_managers import update_ble_peripheral  #  Make sure this import is correct
...
def on_spec(client, userdata, msg):
    ...
    backend_data_topic = f"/gppr_doppelio/{device_id}/data"
    
    mqtt.subscribe_sync(agent_data_topic, on_data_message)
    mqtt.subscribe_sync(backend_data_topic, on_data_message)  # ✅ ADD THIS
